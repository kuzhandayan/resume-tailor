...
- **AI/ChromaDB semantic search — LOCATED, but not your build.** Lives on the `develop` branch (not yet merged to `live`) under `src/routers/agentRouters/` — the "AgentGoto" module: ChromaDB vector collections (inventory/properties), OpenAI `text-embedding-3-small` embeddings, MCP tool endpoints. Real and confirmed. Your involvement: 3 commits, all merges of a teammate's (`zunaith-propgoto`, 42/50 commits — primary author) `feature/agent_goto_embed` branch — you merged/approved it, you did not build it. Do not claim "built" or "designed" this subsystem; if mentioned at all, frame as "reviewed and merged" or leave off entirely and rely on the separately-verified "Agent Goto" hackathon project (Cityscape Global 2025 Finalist) for AI/MCP credibility instead, since that one is genuinely yours.
...
- **CORRECTED — the headline metric doesn't exist in the repo:** exhaustively searched all code, docs, tests, and commit messages for any benchmark, load test, or logged timing — nothing. There is no test suite at all (repo docs explicitly say so). **Already fixed on `base_stable_template.tex`** — the fabricated number was removed and replaced with a qualitative description of the vectorized pipeline.
- **User's own informal observation (per interview, not a formal benchmark):** ~10,000 records imported in under 20 seconds, observed directly by the user during real import work. This is a real, personally-witnessed data point — stronger than nothing — but it's not a logged/reproducible benchmark, so **per user's explicit instruction, no number goes on the resume** even though this one is closer to true. Keep this only as private interview-prep context: if asked "roughly how fast is bulk import," you have a real number to reference from firsthand experience, just don't put it in writing on the resume itself.
...
### Collections & reconciliation — VERIFIED, located
- **Located:** `backend-server`, `live` branch, two route folders — `all_wallet_report` and `receipt`.
- **`all_wallet_report` — strong, near-primary ownership: 29 of 35 commits (~83%).** Real, evidenced work: reconciliation report generation (PDF/Excel), Arabic/English bilingual templates, active-unit filtering, currency/date formatting fixes. This is the "filterable manager dashboard with live collected/unsettled totals" and reporting half of the portfolio's Collections claim.
- **`receipt` — thin, do not claim: only 2 of 62 commits (~3%).** The receipt-tracing (proxy vs real receipts, wallet→property→agreement→invoice→receipt chain logic itself) was built by others.
- **Resume-ready bullet (honest scope):** "Built wallet/collections reconciliation reporting (`all_wallet_report`) — bilingual (Arabic/English) PDF and Excel exports with active-unit filtering — for the platform's finance module."
...
**Follow-up investigation (resolved):**
- ChromaDB/AI semantic search: located on `develop` branch, `src/routers/agentRouters/` (AgentGoto module) — real, but user's role is 3 merge commits of a teammate's branch, not authorship. Do not claim as built.
- Collections & reconciliation: located in `backend-server`/`live`, split across `all_wallet_report` (user owns ~83%, real) and `receipt` (user has ~3%, not theirs). Resume can honestly claim the `all_wallet_report` reconciliation-reporting half only.
- RabbitMQ throughput: user has a real informal observation (~10,000 records / <20s from direct experience) but explicitly wants no number on the resume — keep as private interview-prep context only.
